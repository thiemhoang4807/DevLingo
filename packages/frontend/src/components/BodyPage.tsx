...
import { Outlet } from "react-router-dom"; // <-- Thêm thư viện này
...
export default function Body() {
    const listRAT: string[] = [
        "Zero Trust Architecture", "Edge Computing", "GraphQL", "WebAssembly", "DevSecOps"
    ]; 
    const listRAT_li = listRAT.map(el => (
        <li key={el} className="font-[500] font-medium text-[16px] leading-[24px] pt-[12px] marker:text-[#6B7280]">
            <a href="#" className="text-[#3B82F6] pl-[12px]"> {el} </a>
        </li>
    ));

    const listTT: string[] = [
        "LLM (Large Language Model)", "Prompt Engineering", "Generative AI", "Quantum Computing", "Ransomware"
    ]; 
